...
const findExtensionId = async (context) => {
  ...
  const extensions = await extensionsPage.locator("extensions-manager").evaluate((manager) => {
    const visit = (root) => [
      ...root.querySelectorAll("extensions-item")
    ].map((item) => ({
      id: item.id,
  ...
  const extension = extensions.items.find((item) => item.name === "Poe Trade Plus")
  if (!extension?.id) throw new Error(`PoeTradePlus was not loaded by Chrome: ${JSON.stringify(extensions)}`)
  if (extension.state !== "ENABLED") throw new Error(`PoeTradePlus is disabled: ${JSON.stringify(extension)}`)
  return extension.id
}
...
test("Chinese Trade localization runs in the built extension", async () => {
  ...
  try {
    ...
    expect(await extensionPage.evaluate(() => chrome.permissions.contains({
    ...
    await expect(page.locator("#minimum")).toHaveAttribute("placeholder", "最小")
